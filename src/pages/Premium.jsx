...
const FAQS = [
  {
    q: 'How do I pay?',
    a: 'Payment is processed securely via Flutterwave — cards, USSD, bank transfer, and mobile money all supported.',
  },
  ...
  { q: 'What currency are prices in?', a: 'All prices are in US Dollars ($). Flutterwave will convert to your local currency at checkout.' },
...
export default function Premium() {
  ...
  // Flutterwave configs
  const fwConfig = (planId) => ({
    public_key: process.env.REACT_APP_FLW_PUBLIC_KEY,
    tx_ref: `joblify_${user?.uid}_${Date.now()}`,
    amount: planId === 'premium' ? 4 : 40,
    currency: 'USD',
  ...
  const handleFlutterPayment = useFlutterwave(fwConfig('premium'));
  const handleFlutterPaymentAnnual = useFlutterwave(fwConfig('premium-annual'));
  ...
  const handleUpgrade = async (planId) => {
    ...
    const paymentHandler = planId === 'premium' ? handleFlutterPayment : handleFlutterPaymentAnnual;
      ...
      callback: async (response) => {
        ...
        if (response.status === 'successful') {
          try {
            ...
            setError('Payment succeeded but account update failed. Contact support.');
          }
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-20">
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        {success && <div className="mb-8 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-xl text-green-700 text-sm text-center">{success}</div>}
        {error && <div className="mb-8 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded-xl text-red-700 text-sm text-center">{error}</div>}

        {isPremium && (
          <div className="mb-10 p-5 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 rounded-2xl flex items-center gap-4">
            ...
            <div key={plan.id} className={`relative bg-white dark:bg-gray-800 rounded-2xl flex flex-col ${plan.accent ? 'border-2 border-primary-500 shadow-xl' : 'border border-gray-200 dark:border-gray-700'}`}>
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <ul className="space-y-2.5 mb-6 flex-1">
                  {plan.features.map((f) => (
                    <li key={f.text} className={`flex items-start gap-2.5 text-sm ${f.included ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400'}`}>
                      {f.included ? <FiCheck className="text-green-500 mt-0.5" size={15} /> : <FiX className="text-gray-300 mt-0.5" size={15} />}
                      {f.text}
                    </li>
                  ))}
                </ul>
                <button onClick={() => handleUpgrade(plan.id)} disabled={plan.disabled || loading || (isPremium && plan.id === user?.tier)} className={`w-full py-3 rounded-xl font-semibold text-sm ${plan.disabled ? 'bg-gray-100 text-gray-400' : plan.accent ? 'bg-primary-600 hover:bg-primary-700 text-white' : 'bg-purple-600 hover:bg-purple-700 text-white'}`}>
                  {loadingPlan === plan.id ? 'Processing…' : isPremium && plan.id === user?.tier ? '✓ Active Plan' : plan.cta}
                </button>
              </div>
            </div>
          ))}
        ...
        {/* rest of your perks/FAQ stays same */}
      </div>
    </div>
  );
}
