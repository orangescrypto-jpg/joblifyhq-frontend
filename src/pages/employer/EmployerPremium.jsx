    ...
    <div className="space-y-12 pb-16 max-w-6xl mx-auto px-4">
      ...
      <div className="text-center pt-12">
        ...
        <div className="flex items-center justify-center gap-3 mt-6">
          ...
          <span className={`text-sm font-medium ${billing === 'annual'? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>Annual <span className="text-green-600 font-semibold text-xs ml-1">Save 20%</span></span>
        </div>
      </div>
          ...
          return (
            <div key={plan.key} className={`rounded-2xl border-2 ${plan.color} overflow-hidden shadow-sm ${plan.tag? 'md:-mt-4 md:shadow-xl' : ''}`}>
              <div className={`${plan.headerBg} p-6 ${plan.key!== 'basic'? 'text-white' : 'text-gray-900 dark:text-white'}`}>
                ...
                {plan.monthlyPrice === 0? (
                  <div className="mt-3"><span className="text-3xl font-black">Free</span><span className="text-sm ml-1 opacity-70">forever</span></div>
                ) : (
                  <div className="mt-3">
                    <span className="text-3xl font-black">{fmt(displayPrice)}</span>
                    <span className="text-sm ml-1 opacity-70">/month</span>
                    {billing === 'annual' && <p className="text-xs mt-1 opacity-60">Billed {fmt(plan.annualPrice)}/year</p>}
                    {plan.localHint && <p className="text- mt-1 opacity-60">{plan.localHint}</p>}
                  </div>
                )}
              </div>
